...
def main_menu(user_id):
    ...
    row3 = [KeyboardButton("💳 Wallet & Withdraw")]
    markup.row(*row3)
...
def handle_all_messages(message):
    ...
    if user_id in user_states:
        ...
        if state and state.startswith('admin_set_reward_') and is_admin(user_id):
            try:
                val = float(text.strip())
                key = state.replace('admin_set_', '')
                update_setting(key, str(val))
                bot.send_message(user_id, f"✅ Reward for {key} updated to ₹{val}", reply_markup=admin_markup(user_id))
            except ValueError:
                bot.send_message(user_id, "❌ Please enter a valid number.", reply_markup=admin_markup(user_id))
            del user_states[user_id]
            return

        # ==========================================
        # 🔥 GMAIL & MAP STOCK ENTRY STATES
        # ==========================================
        elif state == 'ngm_wait_single' and is_admin(user_id):
            try:
                g_mail, g_pass = text.split('|')
                run_query("INSERT INTO new_gmail_tasks (gmail, password) VALUES (%s, %s)", (g_mail.strip(), g_pass.strip()), commit=True)
                bot.send_message(user_id, f"✅ Successfully Added: <code>{g_mail.strip()}</code>", parse_mode="HTML", reply_markup=admin_markup(user_id))
            except:
                bot.send_message(user_id, "❌ Invalid format. Use: `Gmail | Password`", parse_mode="Markdown")
            del user_states[user_id]
            return

        elif state == 'ngm_wait_bulk_pass' and is_admin(user_id):
            user_states[user_id] = {'state': 'ngm_wait_bulk_gmails', 'pass': text.strip()}
            bot.send_message(user_id, f"✅ Password Saved: <code>{text.strip()}</code>\n\n👉 <b>Step 2:</b> Now send all Gmails, each on a new line:", parse_mode="HTML")
            return

        elif state == 'ngm_wait_bulk_gmails' and is_admin(user_id):
            g_pass = state_data['pass']
            gmails = text.strip().split('\n')
            count = 0
            for g in gmails:
                if g.strip():
                    run_query("INSERT INTO new_gmail_tasks (gmail, password) VALUES (%s, %s)", (g.strip(), g_pass), commit=True)
                    count += 1
            bot.send_message(user_id, f"✅ Successfully added {count} Gmail tasks to stock!", reply_markup=admin_markup(user_id))
            del user_states[user_id]
            return

        elif state == 'ngm_wait_del_id' and is_admin(user_id):
            try:
                tid = int(text.strip())
                run_query("DELETE FROM new_gmail_tasks WHERE id=%s", (tid,), commit=True)
                bot.send_message(user_id, f"✅ Task ID {tid} deleted successfully.", reply_markup=admin_markup(user_id))
            except:
                bot.send_message(user_id, "❌ Invalid ID format.")
            del user_states[user_id]
            return

        elif state == 'map_wait_single' and is_admin(user_id):
            try:
                m_link, m_rev = text.split('|', 1)
                run_query("INSERT INTO map_tasks (link, review_text) VALUES (%s, %s)", (m_link.strip(), m_rev.strip()), commit=True)
                bot.send_message(user_id, "✅ Map task added successfully!", reply_markup=admin_markup(user_id))
            except:
                bot.send_message(user_id, "❌ Invalid format. Use: `Link | Review Text`", parse_mode="Markdown")
            del user_states[user_id]
            return

        elif state == 'map_wait_bulk' and is_admin(user_id):
            lines = text.strip().split('\n')
            count = 0
            for line in lines:
                if '|' in line:
                    m_link, m_rev = line.split('|', 1)
                    run_query("INSERT INTO map_tasks (link, review_text) VALUES (%s, %s)", (m_link.strip(), m_rev.strip()), commit=True)
                    count += 1
            bot.send_message(user_id, f"✅ Successfully added {count} Map tasks to stock!", reply_markup=admin_markup(user_id))
            del user_states[user_id]
            return

        elif state == 'map_wait_del_id' and is_admin(user_id):
            try:
                tid = int(text.strip())
                run_query("DELETE FROM map_tasks WHERE id=%s", (tid,), commit=True)
                bot.send_message(user_id, f"✅ Map Task ID {tid} deleted.", reply_markup=admin_markup(user_id))
            except:
                bot.send_message(user_id, "❌ Invalid ID.")
            del user_states[user_id]
            return

        elif state == 'map_wait_rules' and is_admin(user_id):
            update_setting('map_rules', text.strip())
            bot.send_message(user_id, "✅ Map rules updated successfully!", reply_markup=admin_markup(user_id))
            del user_states[user_id]
            return

        elif state == 'admin_custom_rej_ngm' and is_admin(user_id):
            tid = state_data['tid']
            tgt = state_data['tgt']
            ...
            if is_task_processed('new_gmail_tasks', tid):
                ...
                update_both(user_id, amsg, ach, orig_msg, caption, dash_markup, None, is_photo=True)
    ...
    if message.content_type == 'text':
        if text == "💳 Wallet & Withdraw":
            ...
            tot_wd = run_query("SELECT SUM(amount) FROM approved_withdraws WHERE user_id=%s", (user_id,), fetch='one')[0] or 0.0
            
            pend_query = run_query("SELECT COUNT(id), SUM(amount) FROM pending_withdraws WHERE user_id=%s", (user_id,), fetch='one')
            pend_cnt = pend_query[0] if pend_query else 0
            pend_amt = pend_query[1] if pend_query and pend_query[1] else 0.0
            
            uname_display = message.from_user.username if message.from_user.username else "User"
            name_display = message.from_user.first_name
            min_upi = get_setting('min_upi')

            msg = (f"💰 <b>WALLET & WITHDRAWAL DETAILS</b>\n\n"
                   f"👤 <b>Name:</b> {name_display} | @{uname_display}\n"
                   f"🆔 <b>User ID:</b> <code>{user_id}</code>\n\n"
                   f"💵 <b>Available Balance:</b> ₹{bal:.2f}\n"
                   f"🏆 <b>Total Earned:</b> ₹{tot_earn:.2f}\n"
                   f"💸 <b>Total Withdrawn:</b> ₹{tot_wd:.2f}\n"
                   f"⏳ <b>Pending Withdrawals:</b> {pend_cnt} (₹{pend_amt:.2f})\n\n"
                   f"<i>Click 💸 Withdraw Funds below to start withdrawal (Minimum ₹{min_upi}):</i>")
            
            markup = InlineKeyboardMarkup()
            markup.row(InlineKeyboardButton("💸 Withdraw Funds", callback_data="req_withdraw"))
            markup.row(InlineKeyboardButton("📜 Task History", callback_data="req_task_history"))
            markup.row(InlineKeyboardButton("📺 Watch Withdrawal Tutorial", callback_data="watch_wd_tutorial"))
            ...
            if get_setting('new_gmail_task') == 'OFF' and not is_admin(user_id): 
                bot.send_message(user_id, "❌ <b>This Option Closed By Admin Raka</b>", parse_mode="HTML")
                return
...
def callback_query(call):
    ...
    # ==========================================
    # 🔥 ADMIN FIXES: TOGGLES, REWARDS & PANELS
    # ==========================================
    
    # 1. VISIBILITY TOGGLES (User keyboard buttons hide/show)
    if data == "admin_vis_toggles" and is_admin(user_id):
        markup = InlineKeyboardMarkup()
        for key in ['vis_create_gmail', 'vis_new_gmail', 'vis_old_gmail', 'vis_map', 'vis_withdraw', 'vis_bulk_gmail', 'vis_bet']:
            val = get_setting(key)
            markup.add(InlineKeyboardButton(f"{key.replace('vis_', '').upper()} : {val}", callback_data=f"tglvis_{key}"))
        markup.add(InlineKeyboardButton("🔙 Back", callback_data="adm_panel_settings"))
        bot.edit_message_text("👁️ <b>VISIBILITY TOGGLES</b>\nHide or Show buttons on User Keyboard:", user_id, call.message.message_id, parse_mode="HTML", reply_markup=markup)
        return

    elif data.startswith("tglvis_") and is_admin(user_id):
        key = data.replace("tglvis_", "")
        current = get_setting(key)
        update_setting(key, "OFF" if current == "ON" else "ON")
        call.data = "admin_vis_toggles"
        return callback_query(call)

    # 2. STATUS TOGGLES (On/Off features with Admin Raka msg)
    elif data == "admin_stat_toggles" and is_admin(user_id):
        markup = InlineKeyboardMarkup()
        for key in ['create_gmail_task', 'new_gmail_task', 'old_gmail_task', 'map_review_task', 'withdraw']:
            val = get_setting(key)
            markup.add(InlineKeyboardButton(f"{key.upper()} : {val}", callback_data=f"tglstat_{key}"))
        markup.add(InlineKeyboardButton("🔙 Back", callback_data="adm_panel_settings"))
        bot.edit_message_text("⛔ <b>STATUS TOGGLES</b>\nTurn ON/OFF functions (Sends Admin Raka closed msg):", user_id, call.message.message_id, parse_mode="HTML", reply_markup=markup)
        return

    elif data.startswith("tglstat_") and is_admin(user_id):
        key = data.replace("tglstat_", "")
        current = get_setting(key)
        update_setting(key, "OFF" if current == "ON" else "ON")
        call.data = "admin_stat_toggles"
        return callback_query(call)

    # 3. SET TASK REWARDS MENU
    elif data == "admin_reward_menu" and is_admin(user_id):
        markup = InlineKeyboardMarkup()
        for k in ['reward_gmail', 'reward_oldgmail', 'reward_map', 'reward_newgmail_single', 'reward_newgmail_bulk']:
            markup.add(InlineKeyboardButton(f"{k} = ₹{get_setting(k)}", callback_data=f"set_rew_{k}"))
        markup.add(InlineKeyboardButton("🔙 Back", callback_data="adm_panel_settings"))
        bot.edit_message_text("💰 <b>SET TASK REWARDS</b>\nClick to change reward amounts:", user_id, call.message.message_id, parse_mode="HTML", reply_markup=markup)
        return

    elif data.startswith("set_rew_") and is_admin(user_id):
        key = data.replace("set_rew_", "")
        user_states[user_id] = {'state': f'admin_set_{key}'}
        bot.send_message(user_id, f"👉 Enter new reward amount for <b>{key}</b>:", parse_mode="HTML")
        return

    # 4. FIX SET MIN WITHDRAW
    elif data == "set_min_upi" and is_admin(user_id):
        user_states[user_id] = {'state': 'admin_set_min_upi'}
        bot.send_message(user_id, "👉 Enter minimum <b>UPI</b> withdraw amount (₹):", parse_mode="HTML")
        return
    elif data == "set_min_usdt" and is_admin(user_id):
        user_states[user_id] = {'state': 'admin_set_min_usdt'}
        bot.send_message(user_id, "👉 Enter minimum <b>USDT</b> withdraw amount ($):", parse_mode="HTML")
        return

    # 5. GMAIL PANEL ACTIONS
    elif data == "ngm_add_single" and is_admin(user_id):
        user_states[user_id] = {'state': 'ngm_wait_single'}
        bot.send_message(user_id, "👉 Send Gmail and Password formatted as `Gmail | Password`\nExample: `rohan@gmail.com | rohan123`", parse_mode="Markdown")
        return
    elif data == "ngm_add_bulk" and is_admin(user_id):
        user_states[user_id] = {'state': 'ngm_wait_bulk_pass'}
        bot.send_message(user_id, "👉 <b>Step 1:</b> Send the common <b>Password</b> that will be used for all the bulk Gmails:", parse_mode="HTML")
        return
    elif data == "ngm_manage_id" and is_admin(user_id):
        user_states[user_id] = {'state': 'ngm_wait_del_id'}
        bot.send_message(user_id, "👉 Send the Task ID you want to delete:")
        return
    elif data == "ngm_delete_all" and is_admin(user_id):
        run_query("DELETE FROM new_gmail_tasks", commit=True)
        bot.send_message(user_id, "✅ All New Gmail stock has been deleted!")
        return

    # 6. MAP PANEL ACTIONS
    elif data == "map_add_single" and is_admin(user_id):
        user_states[user_id] = {'state': 'map_wait_single'}
        bot.send_message(user_id, "👉 Send Map Link and Review formatted as `Link | Review Text`\nExample: `https://maps.app.goo.gl/... | Nice place!`", parse_mode="Markdown")
        return
    elif data == "map_add_bulk" and is_admin(user_id):
        user_states[user_id] = {'state': 'map_wait_bulk'}
        bot.send_message(user_id, "👉 Send multiple map tasks, each on a new line formatted as `Link | Review Text`", parse_mode="Markdown")
        return
    elif data == "map_manage_task" and is_admin(user_id):
        user_states[user_id] = {'state': 'map_wait_del_id'}
        bot.send_message(user_id, "👉 Send the Map Task ID you want to delete:")
        return
    elif data == "map_edit_rules" and is_admin(user_id):
        user_states[user_id] = {'state': 'map_wait_rules'}
        bot.send_message(user_id, "👉 Send the new Global Rules for Map Task:")
        return
    ...
    elif data == "watch_wd_tutorial":
        bot.send_message(user_id, "📺 Withdrawal tutorial will be available soon!", parse_mode="HTML")
        return

    # ==========================================
    # DASHBOARD AND ADMIN CORE ACTIONS (MENUS)
    # ==========================================
    elif data == "adm_panel_settings" and is_admin(user_id):
        edit_msg_safe(user_id, call.message.message_id, "⚙️ <b>BOT SETTINGS & CONFIGURATION</b>", get_settings_markup(user_id))
        return
        ...
        edit_msg_safe(user_id, call.message.message_id, f"📧 <b>GMAIL MANAGEMENT PANEL</b>\n━━━━━━━━━━━━━━━━━━━\nAssets in Stock: <b>{avail}</b>", markup)
        return
        ...
        edit_msg_safe(user_id, call.message.message_id, f"🗺️ <b>MAP TASKS PANEL</b>\n━━━━━━━━━━━━━━━━━━━\nAssets in Stock: <b>{avail}</b>", markup)
        return
        ...
        edit_msg_safe(user_id, call.message.message_id, msg, markup)
        return
        ...
        markup = InlineKeyboardMarkup().add(InlineKeyboardButton("🔙 Back to Main Panel", callback_data="admin_back"))
        edit_msg_safe(user_id, call.message.message_id, "📢 <b>BROADCAST MODE</b>\n\nJo message sabko bhejna hai, wo bhejein (Text, Photo with Caption, Video etc):", markup)
        return
        ...
        markup.row(InlineKeyboardButton("🔙 Back to Main Panel", callback_data="admin_back"))
        edit_msg_safe(user_id, call.message.message_id, "💸 <b>MANAGE USER BALANCE</b>\nSelect operation:", markup)
        return

    elif data == "admin_ban_user" and is_admin(user_id):
        user_states[user_id] = {'state': 'admin_wait_ban_uid'}
        markup = InlineKeyboardMarkup().add(InlineKeyboardButton("🔙 Back to Main Panel", callback_data="admin_back"))
        edit_msg_safe(user_id, call.message.message_id, "🚫 <b>BAN / UNBAN USER</b>\n\n👉 Kripya User ka <b>Telegram ID</b> bhejein jise aap Ban ya Unban karna chahte hain:", markup)
        return
        ...
        markup = InlineKeyboardMarkup().add(InlineKeyboardButton("🔙 Back to Main Panel", callback_data="admin_back"))
        if not records:
            edit_msg_safe(user_id, call.message.message_id, "📋 <b>BANNED USERS LIST</b>\n\nKoi bhi user ban nahi hai.", markup)
        else:
        ...
        edit_msg_safe(user_id, call.message.message_id, "🔍 <b>SEARCH USER</b>\n👉 User ka <b>6-Digit Bot ID</b> likh kar bhejein:", markup)
        return
        ...
        edit_msg_safe(user_id, call.message.message_id, "➕ <b>ADD BALANCE</b>\n👉 User ka <b>Telegram ID</b> bhejein jisme balance dalna hai:", markup)
        return

    elif data == "admin_deductbal" and is_admin(user_id):
        user_states[user_id] = {'state': 'admin_wait_deduct_uid'}
        markup = InlineKeyboardMarkup().add(InlineKeyboardButton("🔙 Back", callback_data="admin_manage_bal"))
        edit_msg_safe(user_id, call.message.message_id, "➖ <b>DEDUCT BALANCE</b>\n👉 User ka <b>Telegram ID</b> bhejein jiska balance kaatna hai:", markup)
        return

    elif data == "admin_back" and is_admin(user_id):
        edit_msg_safe(user_id, call.message.message_id, "🛠️ <b>EXECUTIVE DASHBOARD</b>\nPlease select a category:", admin_markup(user_id))
        return
    ...
    # ==========================================
        ...
        edit_msg_safe(user_id, call.message.message_id, "📢 <b>AUTO-ALERT SETUP</b>\nSelect which task's automated broadcast you want to configure:", markup)
        return
        ...
        markup.row(InlineKeyboardButton("🔙 Back to Settings", callback_data="adm_panel_settings"))
        edit_msg_safe(user_id, call.message.message_id, "⚙️ <b>WITHDRAWAL RESTRICTIONS</b>", markup)
        return
        ...
        edit_msg_safe(user_id, call.message.message_id, "⏱️ <b>AUTO-APPROVE TIME SETUP</b>\n\nKripya time set karein (Hours mein).\nJaise: <b>54</b> (2 din 6 ghante ke liye) ya <b>1.5</b> (1 ghanta 30 minute ke liye).", markup)
        return

    elif data == "admin_set_oldpass" and is_admin(user_id):
        user_states[user_id] = {'state': 'admin_wait_old_pass'}
        markup = InlineKeyboardMarkup().add(InlineKeyboardButton("🔙 Cancel", callback_data="adm_panel_settings"))
        edit_msg_safe(user_id, call.message.message_id, "🔑 <b>OLD GMAIL PASSWORD</b>\nNaya password likh kar bhejein jo users ko Old Gmail task mein show hoga:", markup)
        return

    elif data == "admin_set_pass" and is_admin(user_id):
        user_states[user_id] = {'state': 'admin_set_gmail_pass'}
        markup = InlineKeyboardMarkup().add(InlineKeyboardButton("🔙 Cancel", callback_data="adm_panel_settings"))
        edit_msg_safe(user_id, call.message.message_id, "🔑 <b>CREATE GMAIL PASSWORD</b>\nNaya password likh kar bhejein jo users ko Create Gmail task mein show hoga:", markup)
        return

    elif data == "adm_set_submit_msg_new" and is_admin(user_id):
        user_states[user_id] = {'state': 'admin_wait_submit_msg_new'}
        markup = InlineKeyboardMarkup().add(InlineKeyboardButton("🔙 Cancel", callback_data="adm_panel_settings"))
        edit_msg_safe(user_id, call.message.message_id, "📝 <b>NEW GMAIL SUBMIT MSG SETUP</b>\nPlease send the custom text that users will see when they submit a New Gmail Task.\n<i>(HTML tags like <b> bold </b> are supported)</i>", markup)
        return

    elif data == "adm_set_submit_msg_old" and is_admin(user_id):
        user_states[user_id] = {'state': 'admin_wait_submit_msg_old'}
        markup = InlineKeyboardMarkup().add(InlineKeyboardButton("🔙 Cancel", callback_data="adm_panel_settings"))
        edit_msg_safe(user_id, call.message.message_id, "📝 <b>OLD GMAIL SUBMIT MSG SETUP</b>\nPlease send the custom text that users will see when they submit an Old/Create Gmail Task.\n<i>(HTML tags like <b> bold </b> are supported)</i>", markup)
        return

    elif data == "adm_set_welcome" and is_admin(user_id):
        user_states[user_id] = {'state': 'admin_wait_welcome'}
        markup = InlineKeyboardMarkup().add(InlineKeyboardButton("🔙 Cancel", callback_data="adm_panel_settings"))
        edit_msg_safe(user_id, call.message.message_id, "📝 <b>WELCOME TEXT SETUP</b>\nPlease send the text you want to appear below the default /start welcome message.\n<i>(Send 'none' if you want to remove the extra text)</i>", markup)
        return

    elif data == "adm_set_rec_vid" and is_admin(user_id):
        user_states[user_id] = {'state': 'admin_wait_rec_vid'}
        markup = InlineKeyboardMarkup().add(InlineKeyboardButton("🔙 Cancel", callback_data="adm_panel_settings"))
        edit_msg_safe(user_id, call.message.message_id, "📹 <b>RECOVERY VIDEO SETUP</b>\nPlease upload the video file directly here.\n<i>(Send any text to remove the video)</i>", markup)
        return

    elif data == "adm_set_rec_mail" and is_admin(user_id):
        user_states[user_id] = {'state': 'admin_wait_rec_email'}
        markup = InlineKeyboardMarkup().add(InlineKeyboardButton("🔙 Cancel", callback_data="adm_panel_settings"))
        edit_msg_safe(user_id, call.message.message_id, "📧 <b>RECOVERY EMAIL SETUP</b>\nPlease send the Gmail address you want to use for OTP recovery:", markup)
        return

    elif data == "adm_set_rec_pass" and is_admin(user_id):
        user_states[user_id] = {'state': 'admin_wait_rec_pass'}
        markup = InlineKeyboardMarkup().add(InlineKeyboardButton("🔙 Cancel", callback_data="adm_panel_settings"))
        edit_msg_safe(user_id, call.message.message_id, "🔑 <b>RECOVERY PASSWORD SETUP</b>\nPlease send the <b>App Password</b> (16 letters, no spaces) for your recovery email.\n<i>Note: You must generate this from Google Account -> Security -> App Passwords.</i>", markup)
        return

    elif data == "admin_set_warning_photo" and is_admin(user_id):
        user_states[user_id] = {'state': 'admin_wait_warning_photo'}
        markup = InlineKeyboardMarkup().add(InlineKeyboardButton("🔙 Cancel", callback_data="adm_panel_settings"))
        edit_msg_safe(user_id, call.message.message_id, "🖼️ <b>WARNING PHOTO SETUP</b>\n\nKripya wo <b>Photo</b> bhejein jo users ko 'Get New Gmail Task' click karne par rules ke sath dikhegi.\n\n<i>Note: Sirf photo bhejein, text ki zaroorat nahi hai. Agar hatana ho toh koi bhi text bhej de.</i>", markup)
        return
    ...
    # ==========================================
    # ADMIN PANEL: GMAIL REVIEW ACTIONS
    # ==========================================
    elif data.startswith("ngmbuyer_") and is_admin(user_id):
        tid = int(data.split("_")[1])
        ...
        t_data = run_query("SELECT is_checked, reward_amt, auto_approve_time FROM new_gmail_tasks WHERE id=%s", (tid,), fetch='one')
        is_chk = t_data[0]
        ...
        sync_admin_messages(call, 'new_gmail_tasks', tid, caption, dash_markup, orig_markup)

    elif data.startswith("ngmchecked_") and is_admin(user_id):
        tid = int(data.split("_")[1])
        tgt = int(data.split("_")[2])
        if is_task_processed('new_gmail_tasks', tid): 
            return bot.answer_callback_query(call.id, "Already Processed!", show_alert=True)
        
        t_data = run_query("SELECT sended_buyer, reward_amt, auto_approve_time FROM new_gmail_tasks WHERE id=%s", (tid,), fetch='one')
        is_snd = t_data[0]
        amt = t_data[1] if t_data[1] else float(get_setting('reward_newgmail_single'))
        auto_time = t_data[2]
        t_gmail = extract_gmail(call.message)
        ...
        if auto_time and datetime.datetime.now() >= auto_time:
            ...
            sync_admin_messages(call, 'new_gmail_tasks', tid, caption, dash_markup, None)
            return
    ...
    elif data.startswith("ngmappr_") and is_admin(user_id):
    ...
    elif data.startswith("ngmrej_") and is_admin(user_id):
        tid = int(data.split("_")[1])
        tgt = int(data.split("_")[2])
        if is_task_processed('new_gmail_tasks', tid): 
            return bot.answer_callback_query(call.id, "Already Processed!", show_alert=True)
    ...
    elif data.startswith("ngmcustrej_") and is_admin(user_id):
        tid = int(data.split("_")[1])
        tgt = int(data.split("_")[2])
        if is_task_processed('new_gmail_tasks', tid): 
            return bot.answer_callback_query(call.id, "Already Processed!", show_alert=True)
    ...
    elif data.startswith("ngmcfmr_") and is_admin(user_id):
    ...
    # ==========================================
    # ADMIN PANEL: MANUAL GMAIL ACTIONS
    # ==========================================
    elif data.startswith("manbuyer_") and is_admin(user_id):
        tid = int(data.split("_")[1])
        if is_task_processed('manual_gmail_tasks', tid): 
            return bot.answer_callback_query(call.id, "Already Processed!", show_alert=True)
        
        t_u = run_query("SELECT user_id, gmail, is_checked, auto_approve_time FROM manual_gmail_tasks WHERE id=%s", (tid,), fetch='one')
        if not t_u: return
        tgt, t_gmail, is_chk, auto_time = t_u
        ...
        sync_admin_messages(call, 'manual_gmail_tasks', tid, caption, dash_markup, orig_markup)

    elif data.startswith("manchecked_") and is_admin(user_id):
        tid = int(data.split("_")[1])
        if is_task_processed('manual_gmail_tasks', tid): 
            return bot.answer_callback_query(call.id, "Already Processed!", show_alert=True)
        
        t_u = run_query("SELECT user_id, gmail, sended_buyer, auto_approve_time, reward_amt, task_type FROM manual_gmail_tasks WHERE id=%s", (tid,), fetch='one')
        if not t_u: return
        tgt, t_gmail, is_snd, auto_time, amt, ttype = t_u
        ...
        if auto_time and datetime.datetime.now() >= auto_time:
            ...
            dash_markup = InlineKeyboardMarkup().add(InlineKeyboardButton("⏭️ Next Pending Task", callback_data=f"review_pend_man_{tid}"))
            caption = f"✅ Approved (Instantly) | User: <code>{tgt}</code>\n📧 <b>Gmail:</b> <code>{t_gmail}</code>"
            sync_admin_messages(call, 'manual_gmail_tasks', tid, caption, dash_markup, None)
            return
    ...
    elif data.startswith("manappr_") and is_admin(user_id):
        tid = int(data.split("_")[1])
        if is_task_processed('manual_gmail_tasks', tid): 
            return bot.answer_callback_query(call.id, "Already Processed!", show_alert=True)
    ...
    elif data.startswith("manrej_") and is_admin(user_id):
        tid = int(data.split("_")[1])
        if is_task_processed('manual_gmail_tasks', tid): 
            return bot.answer_callback_query(call.id, "Already Processed!", show_alert=True)
    ...
    elif data.startswith("mancustrej_") and is_admin(user_id):
        tid = int(data.split("_")[1])
        if is_task_processed('manual_gmail_tasks', tid): 
            return bot.answer_callback_query(call.id, "Already Processed!", show_alert=True)
    ...
    elif data.startswith("mancfmr_") and is_admin(user_id):
        reason_code = data.split("_")[1]
        tid = int(data.split("_")[2])
        if is_task_processed('manual_gmail_tasks', tid): 
            return bot.answer_callback_query(call.id, "Already Processed!", show_alert=True)
    ...
    # ==========================================
    elif data.startswith("mapchecked_") and is_admin(user_id):
        t_id = int(data.split("_")[1])
        if is_task_processed('map_tasks', t_id): 
            return bot.answer_callback_query(call.id, "Already Processed!", show_alert=True)
        
        t_data = run_query("SELECT auto_approve_time, reward_amt, assigned_to FROM map_tasks WHERE id=%s", (t_id,), fetch='one')
        auto_time = t_data[0]
        amt = t_data[1] if t_data[1] else float(get_setting('reward_map'))
        tgt = t_data[2]
        ...
        if auto_time and datetime.datetime.now() >= auto_time:
            ...
            sync_admin_messages(call, 'map_tasks', t_id, caption, dash_markup, None)
            return
        ...
        sync_admin_messages(call, 'map_tasks', t_id, caption, dash_markup, orig_markup)
    ...
    # ==========================================
    # 🔥 ADMIN WITHDRAW ACTIONS (100% FIXED)
    # ==========================================
        ...
        try:
            ...
            if req:
                ...
                if insert_check:
                    ...
                    if str(clicked_msg_id) == str(amsg):
                        try: bot.edit_message_text(text=caption, chat_id=clicked_chat_id, message_id=clicked_msg_id, parse_mode="HTML", reply_markup=None)
                        except: pass
                    else:
                        try: bot.edit_message_text(text=caption, chat_id=clicked_chat_id, message_id=clicked_msg_id, parse_mode="HTML", reply_markup=dash_markup)
                        except: pass
                        if amsg:
                ...
                try: bot.answer_callback_query(call.id, "Already Processed!", show_alert=True)
                except: pass
        except Exception as e:
        ...
        if req:
            user_states[user_id] = {'state': 'admin_wd_reject_reason', 'pid': pid, 'current_msg_id': call.message.message_id}
