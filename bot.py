...
def free_expired_gmail_tasks():
    query = """
        ...
        WHERE status='PENDING' AND EXTRACT(EPOCH FROM (NOW() - assigned_time)) > 900
    """
...
def main_menu(user_id):
    ...
    gmail_row = []
    if get_setting('create_gmail_task') == 'ON':
        gmail_row.append(KeyboardButton("📧 Create Gmail Task"))
    if get_setting('old_gmail_task') == 'ON':
        gmail_row.append(KeyboardButton("📧 Old Gmail Task"))
    if gmail_row:
        markup.row(*gmail_row)
        
    if get_setting('map_review_task') == 'ON':
        markup.row(KeyboardButton("📍 Get Review Task"))
...
def send_welcome(message):
    ...
    msg = (f"✨ <b>𝗪𝗘𝗟𝗖𝗢𝗠𝗘 𝗧𝗢 𝗣𝗥𝗘𝗠𝗜𝗨𝗠 𝗘𝗔𝗥𝗡𝗜𝗡𝗚𝗦</b> ✨\n\n"
           f"Greetings <b>{message.from_user.first_name}</b>!\nComplete verified micro-tasks and earn real cash instantly.\n\n"
...
def handle_all_messages(message):
    ...
    if user_id in user_states and user_states[user_id].get('state') == 'admin_wait_broadcast' and is_admin(user_id):
        ...
        bot.send_message(user_id, "⏳ <b>Initiating Broadcast...</b>", parse_mode="HTML")
        success, failed = 0, 0
    ...
    if user_id in user_states:
        ...
        if state == 'new_gmail_task_ss':
            if message.content_type == 'photo':
                ...
                markup = InlineKeyboardMarkup()
                markup.row(InlineKeyboardButton("✅ Approve (₹15)", callback_data=f"ngm_appr15_{tid}_{user_id}"), 
                           InlineKeyboardButton("✅ Approve (₹20)", callback_data=f"ngm_appr20_{tid}_{user_id}"))
                markup.row(InlineKeyboardButton("❌ Reject", callback_data=f"ngm_rej_{tid}_{user_id}"))
                
                admin_msg = (f"🔔 <b>NEW GMAIL TASK PROOF</b>\n"
                             f"👤 <b>User:</b> <code>{user_id}</code>\n"
                             f"🔖 <b>Task ID:</b> {tid}\n"
                             f"📧 <b>Gmail ID:</b> <code>{t_gmail}</code>")
                             
                bot.send_photo(OWNER_ID, message.photo[-1].file_id, caption=admin_msg, parse_mode="HTML", reply_markup=markup)
                bot.send_message(user_id, "✅ <b>Proof Submitted!</b>\nWait for admin verification.", parse_mode="HTML", reply_markup=main_menu(user_id))
                del user_states[user_id]
                return
            else:
                bot.send_message(user_id, "❌ Please upload a valid <b>Screenshot (Photo)</b>.")
                return

        # Legacy Gmail Create Task SS
        if state == 'gmail_task_screenshot':
            if message.content_type == 'photo':
                ...
                bot.send_message(user_id, "✅ <b>Submission Received!</b> Pls Wait 24Hrs.", parse_mode="HTML", reply_markup=main_menu(user_id))
                del user_states[user_id]
                return
            else:
        ...
        if state == 'map_task_screenshot':
            if message.content_type == 'photo':
                ...
                bot.send_photo(OWNER_ID, message.photo[-1].file_id, caption=admin_msg, parse_mode="HTML", reply_markup=markup)
                bot.send_message(user_id, "✅ <b>Review Submitted!</b>\nOur team is verifying your submission.", parse_mode="HTML", reply_markup=main_menu(user_id))
                del user_states[user_id]
                return
            else:
                bot.send_message(user_id, "❌ Invalid format. Please upload a clear <b>Screenshot (Photo)</b>.")
    ...
    if message.content_type == 'text':
        if text == "📧 Get New Gmail Task":
            ...
            markup.row(InlineKeyboardButton("📚 Bulk Task (₹20/each)", callback_data="ngm_type_bulk"))
            ...
            msg = (f"🗺️ <b>GOOGLE MAPS REVIEW</b>\n💰 <b>Reward:</b> ₹{reward}\n\n📜 <b>Guidelines:</b>\n{rules}\n\n👇 <i>Accept terms to receive assignment:</i>")
            markup = InlineKeyboardMarkup()
            ...
            if get_setting('withdraw') == 'OFF' and not is_admin(user_id):
                bot.send_message(user_id, "❌ <b>Withdrawals are suspended.</b>", parse_mode="HTML")
                return
            markup = ReplyKeyboardMarkup(resize_keyboard=True)
            markup.row(KeyboardButton("🏦 UPI"), KeyboardButton("🪙 USDT"))
            markup.row(KeyboardButton("📜 Withdraw History"), KeyboardButton("🔙 Back to Main"))
            bot.send_message(user_id, f"💸 <b>FUNDS WITHDRAWAL</b>\n🔹 <b>UPI</b> (Min: ₹{get_setting('min_upi')})\n🔹 <b>USDT</b> (Min: ${get_setting('min_usdt')})", parse_mode="HTML", reply_markup=markup)
            ...
            if check_bal < min_val:
                bot.send_message(user_id, f"❌ <b>Insufficient Funds.</b> Minimum is {min_val}.", parse_mode="HTML", reply_markup=main_menu(user_id))
            else:
                curr = "INR (₹)" if text == "🏦 UPI" else "USDT ($)"
                bot.send_message(user_id, f"📝 Specify amount in <b>{curr}</b>:", parse_mode="HTML", reply_markup=telebot.types.ReplyKeyboardRemove())
        ...
        # --- DYNAMIC STATES ---
            ...
            if st == 'admin_ngm_add_single' and is_admin(user_id):
                try:
                    ...
                    bot.send_message(user_id, "✅ Single New Gmail Added!", reply_markup=main_menu(user_id))
                except: bot.send_message(user_id, "❌ Format error. Use: email | pass")
                del user_states[user_id]

            elif st == 'admin_ngm_add_bulk' and is_admin(user_id):
                lines = text.split('\n')
                added = 0
                for line in lines:
                    if '|' in line:
                        gmail, pwd = line.split('|', 1)
                        run_query("INSERT INTO new_gmail_tasks (gmail, password) VALUES (%s, %s)", (gmail.strip(), pwd.strip()), commit=True)
                        added += 1
                bot.send_message(user_id, f"✅ Bulk Import Done! Added {added} Gmails to stock.", reply_markup=main_menu(user_id))
                del user_states[user_id]
                ...
                try:
                    ...
                    bot.send_message(user_id, f"✅ Deleted Task ID {tid}", reply_markup=main_menu(user_id))
                ...
                try:
                    ...
                    if not task:
                        ...
                        msg = f"🛠️ <b>MANAGE TASK:</b> <code>{tid}</code>\n🔗 {task[0]}\n💬 <code>{task[1]}</code>\n📌 {task[2]}"
                        markup = InlineKeyboardMarkup()
                        markup.row(InlineKeyboardButton("🗑️ Delete", callback_data=f"mdel_{tid}"), InlineKeyboardButton("🔗 Edit Link", callback_data=f"medl_{tid}"))
                        markup.row(InlineKeyboardButton("💬 Edit Text", callback_data=f"medt_{tid}"))
                        bot.send_message(user_id, msg, parse_mode="HTML", reply_markup=markup)
                except: bot.send_message(user_id, "❌ Invalid ID.")
                del user_states[user_id]
                ...
                bot.send_message(user_id, f"✅ Link Updated!", reply_markup=main_menu(user_id))
                del user_states[user_id]
                ...
                bot.send_message(user_id, f"✅ Text Updated!", reply_markup=main_menu(user_id))
                del user_states[user_id]

            elif st == 'admin_set_map_rules' and is_admin(user_id):
                update_setting('map_rules', text)
                bot.send_message(user_id, "✅ Map Rules updated!", reply_markup=main_menu(user_id))
                del user_states[user_id]
                ...
                try:
                    ...
                    bot.send_message(user_id, "✅ Map Task Added!", reply_markup=main_menu(user_id))
                except: bot.send_message(user_id, "❌ Format galat hai.")
                del user_states[user_id]
                ...
                for line in lines:
                    if '|' in line:
                        ...
                        run_query("INSERT INTO map_tasks (link, review_text) VALUES (%s, %s)", (link.strip(), rev_txt.strip()), commit=True)
                        added += 1
                bot.send_message(user_id, f"✅ Added {added} tasks.", reply_markup=main_menu(user_id))
                del user_states[user_id]
                ...
                try:
                    ...
                    del user_states[user_id]
                except: bot.send_message(user_id, "❌ Number dalein.")
                ...
                try:
                    ...
                    bot.send_message(user_id, "✅ Admin Granted!", reply_markup=main_menu(user_id))
                    del user_states[user_id]
                ...
                try:
                    ...
                    bot.send_message(user_id, f"✅ Provide your <b>UPI/BP20 Address</b>:", parse_mode="HTML")
                except:
                    bot.send_message(user_id, "❌ Limit error or insufficient balance.", reply_markup=main_menu(user_id))
                    del user_states[user_id]
                ...
                bot.send_message(user_id, "✅ <b>Request Logged!</b> Will be processed soon.", parse_mode="HTML", reply_markup=main_menu(user_id))
                ...
                try: user_states[user_id] = {'state': 'admin_wait_amt', 'uid': int(text)}; bot.send_message(user_id, "👉 Provide amount in ₹:")
                ...
                try:
                    ...
                    bot.send_message(user_id, "✅ Liquidity routed!", reply_markup=main_menu(user_id))
                    try: bot.send_message(state_data['uid'], f"🎉 Bonus of ₹{text} credited!", parse_mode="HTML")
                    except: pass
                except: pass
                del user_states[user_id]
...
def callback_query(call):
    ...
    if data == "back_to_main":
        ...
        markup = InlineKeyboardMarkup()
        markup.row(InlineKeyboardButton("✅ Continue", callback_data=f"ngm_go_{mode}"), InlineKeyboardButton("❌ Cancel", callback_data="back_to_main"))
        ...
        bot.send_message(user_id, f"🎉 <b>Success!</b> Allocated {len(tasks)} task(s) to you.", parse_mode="HTML")
        ...
        bot.send_message(user_id, "📸 <b>Upload your screenshot proof for this specific Gmail task:</b>", parse_mode="HTML", reply_markup=markup)
        ...
        add_balance(tgt, amt, "New Gmail Task Approved")
        run_query("UPDATE new_gmail_tasks SET status='COMPLETED' WHERE id=%s", (tid,), commit=True)
        try: bot.edit_message_caption(f"✅ Approved (₹{amt}) | ID: {tid} | User: {tgt}", call.message.chat.id, call.message.message_id)
        except: pass
        try: bot.send_message(tgt, f"🎉 <b>Gmail Task Approved!</b> ₹{amt} added.", parse_mode="HTML")
        except: pass

    elif data.startswith("ngm_rej_"):
        tid = int(data.split("_")[2])
        tgt = int(data.split("_")[3])
        run_query("UPDATE new_gmail_tasks SET status='AVAILABLE', assigned_to=NULL, assigned_time=NULL WHERE id=%s", (tid,), commit=True)
        try: bot.edit_message_caption(f"❌ Rejected & Returned to Stock | ID: {tid}", call.message.chat.id, call.message.message_id)
        except: pass
        ...
        if not task:
            bot.answer_callback_query(call.id, "🚫 No Unique Review Tasks Available Right Now!", show_alert=True)
        else:
            t_id, t_link, t_txt = task
            msg = f"🎉 <b>Asset Allocated!</b>\n\n🔗 <b>Target Directory:</b>\n{t_link}\n\n💬 <b>Required Publish Data:</b>\n<code>{t_txt}</code>\n\n👉 <i>Select 'Completed' upon execution.</i>"
            markup = InlineKeyboardMarkup()
            markup.row(InlineKeyboardButton("✅ Mark Completed", callback_data=f"mapdone_{t_id}"), InlineKeyboardButton("❌ Cancel", callback_data=f"mapcancel_{t_id}"))
            try: bot.delete_message(user_id, call.message.message_id)
            except: pass
        ...
        markup = InlineKeyboardMarkup().add(InlineKeyboardButton("🔙 Discard", callback_data="back_to_main"))
        bot.send_message(user_id, "📸 <b>Upload your screenshot proof below:</b>", parse_mode="HTML", reply_markup=markup)
        ...
        markup.row(InlineKeyboardButton("📦 View Stock", callback_data="ngm_view_stock"), InlineKeyboardButton("🗑️ Delete Task", callback_data="ngm_manage_id"))
        ...
        user_states[user_id] = {'state': 'admin_ngm_add_single'}; bot.send_message(user_id, "📝 Send as:\n<code>email@gmail.com | password123</code>", parse_mode="HTML")
    elif data == "ngm_add_bulk" and is_admin(user_id):
        user_states[user_id] = {'state': 'admin_ngm_add_bulk'}; bot.send_message(user_id, "📚 Send bulk list (one per line):\n<code>e1@gmail.com | pass1</code>\n<code>e2@gmail.com | pass2</code>", parse_mode="HTML")
    elif data == "ngm_manage_id" and is_admin(user_id):
        user_states[user_id] = {'state': 'admin_ngm_manage_id'}; bot.send_message(user_id, "🗑️ Enter Task ID to Delete:")
        ...
        if not records: bot.answer_callback_query(call.id, "📦 Stock empty!", show_alert=True); return
        msg = "📦 <b>GMAIL STOCK (Top 20)</b>\n━━━━━━━━━━━━━━━━━━━\n"
        for r in records: msg += f"🆔 <code>{r[0]}</code> | 📧 {r[1]} | 📌 {r[2]}\n"
        ...
        user_states[user_id] = {'state': 'admin_map_add_single'}; bot.send_message(user_id, "📝 Send: <code>Link | Review Text</code>", parse_mode="HTML")
        ...
        tid = int(data.split("_")[1]); run_query("DELETE FROM map_tasks WHERE id=%s", (tid,), commit=True)
        bot.edit_message_text(f"✅ <b>Erased Task {tid}</b>", call.message.chat.id, call.message.message_id, parse_mode="HTML")
    elif data.startswith("medl_") and is_admin(user_id):
        tid = int(data.split("_")[1]); user_states[user_id] = {'state': 'admin_map_edit_link', 'task_id': tid}; bot.send_message(user_id, "🔗 Insert updated Link:", parse_mode="HTML")
    elif data.startswith("medt_") and is_admin(user_id):
        tid = int(data.split("_")[1]); user_states[user_id] = {'state': 'admin_map_edit_text', 'task_id': tid}; bot.send_message(user_id, "💬 Insert updated Text:", parse_mode="HTML")

    elif data.startswith("mappr_") and is_admin(user_id):
        tgt = int(data.split("_")[1]); t_id = int(data.split("_")[2]); rw = float(get_setting('reward_map'))
        add_balance(tgt, rw, "Map Review Approved")
        run_query("UPDATE map_tasks SET status='COMPLETED' WHERE id=%s", (t_id,), commit=True)
        try: bot.send_message(tgt, f"🎉 <b>Validation Complete!</b> ₹{rw} allocated.", parse_mode="HTML")
        except: pass
        ...
        tgt = int(data.split("_")[1]); t_id = int(data.split("_")[2])
        run_query("UPDATE map_tasks SET status='AVAILABLE', assigned_to=NULL WHERE id=%s", (t_id,), commit=True)
        try: bot.send_message(tgt, f"❌ <b>Validation Failed.</b>", parse_mode="HTML")
        except: pass
        ...
        if get_setting('create_gmail_task') == 'OFF' and not is_admin(user_id): return
        markup = InlineKeyboardMarkup().add(InlineKeyboardButton("🔙 Discard", callback_data="back_to_main"))
        bot.send_message(user_id, "📧 Provide generated <b>Gmail Address</b>:", parse_mode="HTML", reply_markup=markup)
        ...
        markup.row(InlineKeyboardButton("🔙 Dashboard", callback_data="admin_back"))
        bot.edit_message_text("💰 <b>REMUNERATION CONFIGURATION</b>", call.message.chat.id, call.message.message_id, parse_mode="HTML", reply_markup=markup)

    elif data.startswith("setrw_") and is_admin(user_id):
        key = data.split("setrw_")[1]
        user_states[user_id] = {'state': f'admin_set_{key}'}; bot.send_message(user_id, "📝 Threshold (₹):")
        ...
        bot.answer_callback_query(call.id, f"Changed!", show_alert=True)
        ...
        markup = InlineKeyboardMarkup()
        markup.row(InlineKeyboardButton("Set UPI (₹)", callback_data="set_min_upi"), InlineKeyboardButton("Set USDT ($)", callback_data="set_min_usdt"))
        markup.row(InlineKeyboardButton("🔙 Dashboard", callback_data="admin_back"))
        ...
        user_states[user_id] = {'state': 'admin_set_min_upi'}; bot.send_message(user_id, "📝 UPI (₹):")
    elif data == "set_min_usdt" and is_admin(user_id):
        user_states[user_id] = {'state': 'admin_set_min_usdt'}; bot.send_message(user_id, "📝 USDT ($):")
        ...
        if not records: bot.answer_callback_query(call.id, "No users registered yet!", show_alert=True); return
        bot.answer_callback_query(call.id, "Fetching all users data...")
        current_msg = f"👥 <b>ALL USER BALANCES ({len(records)})</b>\n━━━━━━━━━━━━━━━━━━━\n"
        for r in records:
            line = f"👤 {r[1] if r[1] else 'Unknown'} | <code>{r[0]}</code> | 💰 ₹{r[2]:.2f}\n"
        ...
        if current_msg:
            markup = InlineKeyboardMarkup().add(InlineKeyboardButton("🔙 Dashboard", callback_data="admin_back"))
        ...
        if not records: bot.answer_callback_query(call.id, "No history!", show_alert=True); return
        msg = "📜 <b>APPROVED WITHDRAWALS HISTORY:</b>\n\n"
        for r in records:
            curr = "₹" if r[1] == "🏦 UPI" else "$"
            msg += f"👤 <code>{r[0]}</code> | {r[1]} | 💰 {curr}{r[3]} | 📌 <code>{str(r[2]).replace('<','').replace('>','')}</code>\n📅 {r[4]}\n\n"
        bot.send_message(user_id, msg[:4000], parse_mode="HTML")

    elif data == "admin_broadcast" and is_admin(user_id):
        bot.send_message(user_id, "📢 <b>BROADCAST MODE</b>\nJo message sabko bhejna hai bhejein:", parse_mode="HTML")
        user_states[user_id] = {'state': 'admin_wait_broadcast'}

    elif data == "admin_addbal" and is_admin(user_id):
        bot.send_message(user_id, "💸 <b>ADD BALANCE</b>\n👉 Telegram ID bhejein:", parse_mode="HTML")
    ...
    # Withdraw Handling
        ...
        if req:
            ...
            if run_query("INSERT INTO approved_withdraws (user_id, method, address, amount, date) VALUES (%s, %s, %s, %s, %s) RETURNING id", (t_user, meth, addr, amt, dt), fetch='id', commit=True):
                try: bot.send_message(t_user, f"🎉 <b>FUNDS DISBURSED!</b> {meth}", parse_mode="HTML")
                except: pass
                try: bot.edit_message_text(f"✅ Routed for <code>{t_user}</code>", call.message.chat.id, call.message.message_id, parse_mode="HTML")
                except: pass
        ...
        if req:
            ...
            try: bot.edit_message_text(f"❌ Dropped for <code>{t_user}</code>", call.message.chat.id, call.message.message_id, parse_mode="HTML")
            except: pass
            run_query("DELETE FROM pending_withdraws WHERE id=%s", (pid,), commit=True)
